    ...
    if f.endswith(".cem"):
        ...
        with open(f, "rb") as cemfile:
            if cemfile.read(4) != cem_magic:
                ...
                num_faces = int.from_bytes(cemfile.read(4), byteorder='little', signed=False)
                num_vertices = int.from_bytes(cemfile.read(4), byteorder='little', signed=False)
                ...
                result.append( [ f, num_cem, num_child_models, version_maj, version_min, num_faces, num_vertices, num_tag_points, num_materials, num_frames, num_lod_levels ] )
