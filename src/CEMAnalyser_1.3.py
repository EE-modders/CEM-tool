...
@author: zocker_160
@comment: made for CEM version 1.3
"""
    ...
    if f.endswith(".cem"):
        ...
        with open(f, "rb") as cemfile:
            if cemfile.read(4) != cem_magic:
                ...
                unknown_1 = int.from_bytes(cemfile.read(4), byteorder='little', signed=False)  
                unknown_2 = int.from_bytes(cemfile.read(4), byteorder='little', signed=False)  

                vertices_uni = int.from_bytes(cemfile.read(4), byteorder='little', signed=False)  
                faces = int.from_bytes(cemfile.read(4), byteorder='little', signed=False)  
                unknown_3 = int.from_bytes(cemfile.read(4), byteorder='little', signed=False)  
                uvs_uni = int.from_bytes(cemfile.read(4), byteorder='little', signed=False)  
                ...
                name_length = int.from_bytes(cemfile.read(4), byteorder='little', signed=False)  

                cemfile.seek(0)
                num_cem = get_num_CEM_parts2(cemfile.read(-1))
                result.append( [ f, num_cem, version_maj, version_min, unknown_1, unknown_2, vertices_uni, faces, unknown_3, uvs_uni, num_tag_points, num_child_models, name_length ] )
